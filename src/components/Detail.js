...
const Detail = () => {
  ...
  useEffect(() => {
    // this prevents user from accessing the "/detail/:id" route before logging in
    if (!userName) {
    ...
    db.collection("movies")
      ...
      .then(doc => {
        if (doc.exists) {
          ...
          console.log("No such document in firebase found");
        }
      })
...
const GroupWatch = styled.div`
  ...
  div {
    ${
      "" /* height and width of div ciricle is smaller due to which the white border appears */
    }
    height: 40px;
    width: 40px;
